  ...
  if (view === "sos") {
    return (
      <div className="min-h-screen bg-rose-50 p-8 flex flex-col items-center justify-center text-center">
        ...
        <h1 className="text-4xl font-black text-rose-700 mb-4">
          EMERGENCY SOS
        </h1>
        <p className="text-xl text-rose-800 max-w-md mb-8">
          If you are experiencing severe breathing difficulty or chest pain,
        ...
        <div className="space-y-4 w-full max-w-sm">
          <a
            href="tel:102"
            className="block w-full bg-rose-600 text-white font-bold py-6 rounded-2xl text-2xl shadow-xl hover:bg-rose-700"
          >
            Call Ambulance (102)
          </a>
          <button
            onClick={() => setView("home")}
            className="block w-full bg-white text-slate-500 font-bold py-4 rounded-2xl border border-slate-200"
          >
            Return to Dashboard
          </button>
        </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-2 bg-slate-50 px-3 py-1.5 rounded-full border border-slate-200">
              ...
              <p className="text-xs font-bold text-slate-600">
                {envData?.locationName || "Locating..."}
              </p>
            </div>
            {userEmail && (
              <div className="flex items-center gap-2 bg-slate-100 px-3 py-1.5 rounded-full border border-slate-200 text-xs font-semibold text-slate-600">
                {userEmail}
              </div>
            )}
            {onLogout && (
              <button
                onClick={() => onLogout()}
                className="text-xs font-bold text-rose-500 bg-rose-50 px-3 py-1.5 rounded-full border border-rose-100 hover:bg-rose-100"
              >
                Logout
              </button>
            )}
